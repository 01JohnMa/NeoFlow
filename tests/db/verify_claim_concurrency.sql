...
DECLARE
  ...
  v_workers int;
BEGIN
  SELECT count(*) INTO v_duplicated FROM processing_jobs WHERE attempts <> 1;
  IF v_duplicated <> 0 THEN
    RAISE EXCEPTION '并发认领重复: % 个 Job 的 attempts <> 1', v_duplicated;
  END IF;

  SELECT count(*) INTO v_processing FROM processing_jobs WHERE status = 'processing';
  IF v_processing <> 30 THEN
    RAISE EXCEPTION '认领数量异常: 期望 30，实际 %', v_processing;
  END IF;

  SELECT count(DISTINCT locked_by) INTO v_workers
  FROM processing_jobs WHERE status = 'processing' AND locked_by IS NOT NULL;
  IF v_workers < 1 THEN
