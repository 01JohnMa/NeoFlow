---
name: 多租户配置化系统
overview: 实现多租户 + 配置化模板系统，支持质量管理中心和电联接等多部门使用同一套系统，各部门数据隔离、飞书配置独立、字段模板可定制。
todos:
  - id: db-design
  ...
  - id: frontend-upload
    ...
    status: completed
  - id: init-data
    content: 初始化质量管理中心和照明事业部的租户及模板配置数据
    status: completed
  - id: data-migration
    content: 数据迁移脚本：现有用户和文档归属到质量管理中心
    status: completed
  - id: admin-backend
...
- 现有用户的 tenant_id 设为质量管理中心（默认租户）
- 现有文档的 tenant_id 设为质量管理中心
- 迁移脚本需要在新表创建后执行
