...
--
-- 并发前提（接受的残差）：
--   commit_parse_job 的写入隔离依赖 PostgreSQL 行锁（FOR UPDATE）与
--   claim_next_processing_job 的 SKIP LOCKED 语义；失效令牌校验与
--   results(job_id) 规范产物唯一索引作为兜底。不再为"交卷持锁期间不可
--   重领"专门建设双连接暂停测试（验证的是 PG 原生锁语义，残余场景
--   限于多副本 worker + 超长网络分区）。若 worker 副本数 >1 或租约
--   阈值被调低，应把该确定性测试升级为必测。
-- ============================================================
