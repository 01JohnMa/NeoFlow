-- Extract ParseResult belongs to the current Extract Job.
-- Never accept a ParseResult produced by another Job.
...
-- Atomically persist and bind the ParseResult produced by this Extract Job.
-- This closes the crash window between INSERT results and bind_extract_parse_result.
CREATE OR REPLACE FUNCTION commit_extract_parse_result(
    p_job_id UUID,
    p_worker_id TEXT,
    p_attempts INT,
    p_parse_data JSONB,
...
RETURNS TABLE (out_status TEXT, out_parse_result_id UUID, out_reason TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
...
REVOKE EXECUTE ON FUNCTION commit_extract_parse_result(UUID, TEXT, INT, JSONB, JSONB) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION commit_extract_parse_result(UUID, TEXT, INT, JSONB, JSONB) TO service_role;
SELECT pg_notify('pgrst', 'reload schema');
